...
fluidized bed reactor. Conversion times for each bin based on loblolly pine.
...
# load particle size distribution from Peter at NREL
file1 = 'sizeinfo.txt'
...
def cstr(T, nstages, taus, taug, tv, yfw):
    """CSTR reactor model"""
    ...
    # kinetics parameters
    phi = 0.78          # Max tar yield fraction
...
# devolatilization (conversion) time for each bin
# estimated from loblolly pine center temperature profile
tv95 = [0.156, 0.216, 0.35, 0.536, 0.678, 0.952, 1.352, 1.94, 1.93]
...
def despine():
    ...
    py.tick_params(bottom=False, top=False, left=False, right=False)
    py.grid()
...
py.axvline(0.71, c='b', ls='--', lw=2, label='0.5 mm exp')
py.axvline(0.64, c='g', ls='--', lw=2, label='2.0 mm exp')
py.xlim([0, 0.80])
py.xlabel('Tar Yield (wt. fraction)')
py.ylabel('Reactor Height (stage number)')
py.title('Loblolly Pine (model) vs Reference Pine (exp)')
py.legend(loc='best', numpoints=1, fontsize='medium')
despine()
