...
def plot_geldart(dp, rhog, rhos):
    ...
    rhodiff = rhos - rhog
    ax.plot(dp, rhodiff, 'o', color='salmon')
...
def plot_geldart_region(rhog, region):
    """
    Plot a region on the Geldart chart [Geldart1973]_. Data points for
    demarcation lines and regions were digitized from [Khawaja2015]_.
    ...
    rhog : float
        Gas density [g/cm³]
    region : dict
        Points representing region on Geldart chart. Dictionary must include
        minimum diameter `dp_min`, maximum diameter `dp_max`, minimum solid
        density `rhos_min`, and maximum solid density `rhos_max`. Diameters
        are [µm] and densities are [g/cm³].

    Returns
    -------
    Matplotlib figure representing Geldart chart and user defined region.

    Example
    -------
    >>> region = {'dp_min': 300, 'dp_max': 500, 'rhos_min': 2, 'rhos_max': 2.5}
    >>> plot_geldart_region(rhog, region)
    ...
    dp_min = region['dp_min']
    dp_max = region['dp_max']
    rho_diff_min = region['rhos_min'] - rhog
    rho_diff_max = region['rhos_max'] - rhog
    x = [dp_min, dp_min, dp_max, dp_max]
    y = [rho_diff_min, rho_diff_max, rho_diff_max, rho_diff_min]
    ax.fill(x, y, alpha=0.8, color='salmon')
