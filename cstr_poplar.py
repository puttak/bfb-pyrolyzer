...
fluidized bed reactor. Conversion times based on yellow poplar.
...
tv95 = [0.134, 0.186, 0.3, 0.46, 0.582, 0.818, 1.164, 1.676, 1.668]
...
py.axvline(0.65, c='g', ls='--', lw=2, label='2.0 mm sieve')
py.xlim([0, 0.80])
py.xlabel('Tar Yield (wt. fraction)')
py.ylabel('Reactor Height (stage number)')
py.title('Yellow Poplar (model) vs Hybrid Poplar (exp)')
...
py.plot(tar05_sum, ns, lw=2, label='0.5 mm model')
py.plot(tar20_sum, ns, lw=2, label='2.0 mm model')
py.axvline(0.71, c='g', ls='--', lw=2, label='2.0 mm sieve')
py.xlim([0, 0.80])
py.xlabel('Tar Yield (wt. fraction)')
py.ylabel('Reactor Height (stage number)')
py.title('Yellow Poplar (model) vs Tulip Poplar (exp)')
py.legend(loc='best', numpoints=1, fontsize='medium', frameon=False)
despine()
